...
factorial = App (Fix (Abs "f" (Arrow Nat (Arrow Nat Nat)) 
                (Abs "m" Nat (If (IsZero (Var "m")) (Succ Zero) (times (Var "m") (App (Var "f") (Pred (Var "m"))))))))
...
----------------------------------------------------------------
                pattern : (X_1 U ... U X_n)
-}
pwRules :: Pattern -> Set.Set Id
pwRules (VarP x) = Set.singleton x
pwRules (RecP m) | premise m = foldl (\x y -> x `Set.union` freeVarsP y) Set.empty m
